...
*/
auto GME_ModsUndoMod(HWND hpb, const std::vector<GME_BckEntry_Struct>& bckentry_list) -> void
{
        ...
        if (bckentry_list[i].action == GME_BCK_RESTORE_DELETE)
        {
            if (bckentry_list[i].isdir)
            {
                if (GME_IsDir(dst_path))
                {
                    ...
                    {
                        GME_Logs(GME_LOG_WARNING, "GME_ModsUndoMod", "Unable to delete dir",
                                 GME_StrToMbs(dst_path).c_str());
                    }
                }
            } else
            {
                if (GME_IsFile(dst_path))
                {
                    ...
                    {
                        GME_Logs(GME_LOG_WARNING, "GME_ModsUndoMod", "Unable to delete file",
                                 GME_StrToMbs(dst_path).c_str());
                    }
                }
            }
...
*/
auto GME_ModsApplyMod(HWND hpb, const std::wstring& name, int type) -> void
{
    ...
    if (hnd != INVALID_HANDLE_VALUE)
    {
        do
        {
            if (!(fdw.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
            {
                ...
                {
                    /* create LE message... */
                    olap_msg = L"The Mod '" + name + L"' files overlap with the already installed Mod: '" +
                               GME_FilePathToName(fdw.cFileName) + L"'.";
                    olap_msg += L"\n\nOverlapped file(s):\n";
                    for (unsigned k = 0; k < olap_flist.size(); k++)
                        olap_msg += L"  " + olap_flist[k] + L"\n";
                    olap_msg += L"\nThis will alter already installed Mod files, do you want to install it anyway ?";

                    GME_Logs(GME_LOG_WARNING, "GME_ModsApplyMod", "Mod overlaps with",
                    ...
                    if (IDYES != GME_DialogWarningConfirm(g_hwndMain, olap_msg))
                    {
                        ...
                        FindClose(hnd);
                        SendMessage(hpb, PBM_SETPOS, (WPARAM)0, 0);
                        GME_Logs(GME_LOG_WARNING, "GME_ModsApplyMod", "Mod apply canceled by user",
                                 GME_StrToMbs(name).c_str());
                        return;
                    }
                }
            }
        ...
        if (mod_tree->currChild()->isDir())
        {
            ...
            if (GME_IsDir(game_path + rel_path))
            {
                ...
                if (!GME_IsDir(dst_path))
                {
                    ...
                    {
                        GME_DialogError(
                            g_hwndMain, L"Backup creation error for Mod '" + name + L"', the Mod cannot be installed.");
                        GME_Logs(GME_LOG_ERROR, "GME_ModsApplyMod", "Unable to backup dir",
                                 GME_StrToMbs(dst_path).c_str());
        ...
        } else
        {
            ...
            if (GME_IsFile(game_path + rel_path))
            {
                ...
                if (!GME_FileCopy(src_path, dst_path, false))
                {
                    GME_DialogError(
                        g_hwndMain, L"Backup creation error for Mod '" + name + L"', the Mod cannot be installed.");
                    GME_Logs(GME_LOG_ERROR, "GME_ModsApplyMod", "Unable to backup file",
                             GME_StrToMbs(src_path).c_str());
                    GME_ModsUndoMod(hpb, bckentry_list);
                    delete mod_tree;
                    return;
                }
    ...
    if (is_zip_mod)
    {
        ...
        {
            GME_DialogError(
                g_hwndMain, L"Mod-Archive '" + name + L"' Zip extraction error, the Mod cannot be installed.");
            GME_Logs(GME_LOG_ERROR, "GME_ModsApplyMod", "Zip reader init (extraction) failed",
                     GME_StrToMbs(name).c_str());
            delete mod_tree;
            SendMessage(hpb, PBM_SETPOS, (WPARAM)0, 0);
            GME_ModsUndoMod(hpb, bckentry_list);
    ...
    while (mod_tree->nextChild())
    {
        ...
        if (mod_tree->currChild()->isDir())
        {
            ...
            if (!GME_IsDir(dst_path))
            {
                if (!GME_DirCreate(dst_path))
                {
                    ...
                    GME_DialogError(
                        g_hwndMain, L"Create directory error for Mod '" + name + L"', the Mod cannot be installed.");
        ...
        } else
        {
            ...
            {
                /* extract to file */
                if (!mz_zip_reader_extract_to_file(&za, mod_tree->currChild()->getId(), GME_StrToMbs(dst_path).c_str(),
                                                   0))
                {
                    mz_zip_reader_end(&za);
                    GME_DialogError(
                        g_hwndMain, L"Mod-Archive '" + name + L"' Zip extraction error, the Mod cannot be installed.");
                    GME_Logs(GME_LOG_ERROR, "GME_ModsApplyMod", "Zip extract to file (extraction) failed",
                             GME_StrToMbs(mod_tree->currChild()->getPath(true)).c_str());
                    delete mod_tree;
                    SendMessage(hpb, PBM_SETPOS, (WPARAM)0, 0);
                    GME_FileDelete(bck_file);
                    GME_ModsUndoMod(hpb, bckentry_list);
            ...
            } else
            {
                ...
                if (!GME_FileCopy(mod_tree->currChild()->getSource(), dst_path, true))
                {
                    mz_zip_reader_end(&za);
                    GME_DialogError(
                        g_hwndMain, L"File copy error for Mod '" + name + L"', the Mod cannot be installed.");
                    GME_Logs(GME_LOG_ERROR, "GME_ModsApplyMod", "File copy failed",
                             GME_StrToMbs(mod_tree->currChild()->getSource()).c_str());
                    delete mod_tree;
                    SendMessage(hpb, PBM_SETPOS, (WPARAM)0, 0);
                    GME_FileDelete(bck_file);
                    GME_ModsUndoMod(hpb, bckentry_list);
...
*/
auto GME_ModsRestoreMod(HWND hpb, const std::wstring& name) -> void
{
        ...
        if (bckentry_list[i].action == GME_BCK_RESTORE_DELETE)
        {
            if (bckentry_list[i].isdir)
            {
                if (GME_IsDir(dst_path))
                {
                    if (!GME_DirRemove(dst_path))
                    {
                        GME_Logs(GME_LOG_WARNING, "GME_ModsRestoreMod", "Unable to delete dir",
                                 GME_StrToMbs(dst_path).c_str());
                        got_error = true;
                    }
                } else { }
            } else
            {
                if (GME_IsFile(dst_path))
                {
                    if (!GME_FileDelete(dst_path))
                    {
                        GME_Logs(GME_LOG_WARNING, "GME_ModsRestoreMod", "Unable to delete file",
                                 GME_StrToMbs(dst_path).c_str());
                        got_error = true;
                    }
                } else
                {
                    GME_Logs(GME_LOG_NOTICE, "GME_ModsRestoreMod", "Unable to delete file (it does not exists)",
                             GME_StrToMbs(dst_path).c_str());
                }
            }
        } else
    ...
    if (got_error)
    {
        GME_DialogWarning(
            g_hwndMain,
            L"Restore process for Mod '" + name +
            L"' encountered one or more error, please see log text for more details.");
    }
}
...
*/
auto GME_ModsCleanBackup() -> void
{
    GME_Logs(GME_LOG_NOTICE, "GME_ModsCleanBackup", "Cleaning backup tree",
             GME_StrToMbs(GME_GameGetCurBackPath()).c_str());
    ...
    if (hnd != INVALID_HANDLE_VALUE)
    {
        do
        {
            if (!(fdw.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
            {
                ...
                {
                    GME_Logs(GME_LOG_ERROR, "GME_ModsCleanBackup", "Unable to open backup file",
                             GME_StrToMbs(bck_file).c_str());
                }
            }
        } while (FindNextFileW(hnd, &fdw));
    }
    ...
    while (back_tree->nextChild())
    {
        if (!back_tree->currChild()->isDir())
        {
            ...
            if (!is_depend)
            {
                ...
                if (!GME_FileDelete(dst_path))
                {
                    GME_Logs(GME_LOG_WARNING, "GME_ModsCleanBackup", "Unable to delete file",
                             GME_StrToMbs(dst_path).c_str());
                }
            }
        } else
    ...
    while (i--)
    {
        ...
        if (PathIsDirectoryEmptyW(dst_path.c_str()))
        {
            if (!GME_DirRemove(dst_path))
            {
                GME_Logs(GME_LOG_WARNING, "GME_ModsCleanBackup", "Unable to delete dir",
                         GME_StrToMbs(dst_path).c_str());
            }
        }
    }
...
*/
auto GME_ModsToggleSel(int action) -> bool
{
    if (!GME_ModsProc_IsReady())
    {
        GME_DialogWarning(
            g_hwndMain,
            L"Mod(s) installation is currently processing, please wait until current process finish before enabling or disabling mod(s).");
        return false;
    }
...
*/
auto GME_ModsToggleAll(int action) -> bool
{
    if (!GME_ModsProc_IsReady())
    {
        GME_DialogWarning(
...
auto GME_ModsImport() -> bool
{
    ...
    for (unsigned i = 0; i < fmod_list.size(); i++)
    {
        ...
        {
            GME_DialogWarning(
                g_hwndMain, L"The file '" + fmod_list[i] + L"' is not a valid Mod-Archive and will not be imported.");
        }
    }
    ...
    for (unsigned i = 0; i < name_list.size(); i++)
    {
        ...
        case 0:
            if (IDYES == GME_DialogWarningConfirm(
                    g_hwndMain, L"Are you sure you want to move the Mod '" + name_list[i] + L"' to Recycle Bin ?"))
            {
...
*/
auto GME_ModsListQuickEnable(const std::wstring& name, bool enable) -> void
{
    HWND hlv = GetDlgItem(g_hwndMain, LVM_MODSLIST);
...
auto GME_ModsUpdList() -> bool
{
    ...
    if (!GME_IsDir(GME_GameGetCurModsPath()))
    {
        GME_DialogWarning(
            g_hwndMain,
            L"The Mods stock folder '" + GME_GameGetCurModsPath() + L"' does not exists, you should create it.");
    }
...
*/
auto WINAPI GME_ModsUninstall_Th(void* args) -> DWORD
{
...
*/
auto CALLBACK GME_DlgUninst(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) -> BOOL
{
    switch (uMsg)
    {
    case WM_INITDIALOG:
...
*/
auto WINAPI GME_ModsMake_Th(void* args) -> DWORD
{
    ...
    while (zip_root->nextChild())
    {
        ...
        if (zip_root->currChild()->isDir())
        {
            ...
            if (!mz_zip_writer_add_mem(&za, a_name.c_str(), NULL, 0, arg->zip_level))
            {
                ...
                GME_Logs(GME_LOG_ERROR, "GME_ModsMake_Th", "mz_zip_writer_add_mem (dir) failed",
                         GME_StrToMbs(tmp_path).c_str());
                GME_DialogError(g_hwndNewAMod, L"An error occurred during Mod-Archive creation.");
                ShowWindow(GetDlgItem(g_hwndNewAMod, BTN_CREATE), true);
                ShowWindow(GetDlgItem(g_hwndNewAMod, IDCANCEL), false);
                EnableWindow(GetDlgItem(g_hwndNewAMod, BTN_CLOSE), true);
        ...
        } else
        {
            ...
            if (!zip_root->currChild()->getSource().empty())
            {
                ...
                if (!mz_zip_writer_add_file(&za, a_name.c_str(), f_name.c_str(), NULL, 0, arg->zip_level))
                {
                    ...
                    delete arg;
                    GME_FileDelete(tmp_path);
                    GME_Logs(GME_LOG_ERROR, "GME_ModsMake_Th", "mz_zip_writer_add_file failed",
                             GME_StrToMbs(tmp_path).c_str());
                    GME_DialogError(g_hwndNewAMod, L"An error occurred during Mod-Archive creation.");
                    ShowWindow(GetDlgItem(g_hwndNewAMod, BTN_CREATE), true);
                    ShowWindow(GetDlgItem(g_hwndNewAMod, IDCANCEL), false);
            ...
            {
                if (!mz_zip_writer_add_mem(&za, a_name.c_str(), zip_root->currChild()->getData(),
                                           zip_root->currChild()->getDataSize(), arg->zip_level))
                {
                    mz_zip_writer_end(&za);
                    delete zip_root;
                    delete arg;
                    GME_FileDelete(tmp_path);
                    GME_Logs(GME_LOG_ERROR, "GME_ModsMake_Th", "mz_zip_writer_add_mem (mem) failed",
                             GME_StrToMbs(tmp_path).c_str());
                    GME_DialogError(g_hwndNewAMod, L"An error occurred during Mod-Archive creation.");
                    ShowWindow(GetDlgItem(g_hwndNewAMod, BTN_CREATE), true);
                    ShowWindow(GetDlgItem(g_hwndNewAMod, IDCANCEL), false);
...
*/
auto GME_ModsMakeArchive(const std::wstring& src_dir, const std::wstring& dst_path, const std::wstring& desc, int vmaj,
                         int vmin, int vrev, int zlevel) -> void
{
    if (g_ModsMake_Running)
    {
    ...
    if (GME_IsFile(zip_path))
    {
        if (IDCANCEL == GME_DialogQuestionConfirm(g_hwndNewAMod,
                                                  L"The destination file '" + zip_path +
                                                  L"' already exists, do you want to overwrite ?"))
            return;
    }
...
*/
auto GME_ModsMakeArchiveCur(const std::wstring& desc, int vmaj, int vmin, int vrev, int zlevel) -> void
{
    HWND hlv = GetDlgItem(g_hwndMain, LVM_MODSLIST);
...
*/
auto GME_ModsProc_PushApply(const std::wstring& name, int type) -> void
{
    /* build arg list */
...
*/
auto GME_ModsProc_PushRestore(const std::wstring& name) -> void
{
    /* build arg list */
    GME_ModsProc_Arg_Struct args;
...
*/
auto GME_ModsProc_IsReady() -> bool
{
...
*/
auto WINAPI GME_ModsProc_Th(void* args) -> DWORD
{
    ...
    for (unsigned i = 0; i < g_ModsProc_ArgList.size(); i++)
    {
        ...
        {
            GME_ModsApplyMod(GetDlgItem(g_hwndMain, PBM_MODPROC), g_ModsProc_ArgList[i].name,
                             g_ModsProc_ArgList[i].type);
